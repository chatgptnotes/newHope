 ...
 *   If youâ€™re defining a â€˜Profile belongsTo Userâ€™ relationship, the className key should equal â€˜User.â€™
 ...
 *   value for this key is the underscored, singular name of the other model, suffixed with â€˜_idâ€™.
 * - `conditions`: An SQL fragment used to filter related model records. Itâ€™s good
 *   practice to use model names in SQL fragments: â€œUser.active = 1â€� is always
 *   better than just â€œactive = 1.â€�
 ...
 *   decrease the â€œ[singular_model_name]_countâ€� field in the foreign table whenever you do
 ...
 *
 * - `className`: the classname of the model being associated to the current model.
 *   If youâ€™re defining a â€˜User hasOne Profileâ€™ relationship, the className key should equal â€˜Profile.â€™
 ...
 *   current model, suffixed with â€˜_idâ€™. In the example above it would default to 'user_id'.
 * - `conditions`: An SQL fragment used to filter related model records. Itâ€™s good
 *   practice to use model names in SQL fragments: â€œProfile.approved = 1â€� is
 *   always better than just â€œapproved = 1.â€�
 * - `fields`: A list of fields to be retrieved when the associated model data is
 *   fetched. Returns all fields by default.
 * - `order`: An SQL fragment that defines the sorting order for the returned associated rows.
 * - `dependent`: When the dependent key is set to true, and the modelâ€™s delete()
 ...
 *
 * - `className`: the classname of the model being associated to the current model.
 *   If youâ€™re defining a â€˜User hasMany Commentâ€™ relationship, the className key should equal â€˜Comment.â€™
 * - `foreignKey`: the name of the foreign key found in the other model. This is
 *   especially handy if you need to define multiple hasMany relationships. The default
 *   value for this key is the underscored, singular name of the actual model, suffixed with â€˜_idâ€™.
 * - `conditions`: An SQL fragment used to filter related model records. Itâ€™s good
 *   practice to use model names in SQL fragments: â€œComment.status = 1â€� is always
 *   better than just â€œstatus = 1.â€�
 * - `fields`: A list of fields to be retrieved when the associated model data is
 *   fetched. Returns all fields by default.
 * - `order`: An SQL fragment that defines the sorting order for the returned associated rows.
 ...
 *
 * - `className`: the classname of the model being associated to the current model.
 *   If you're defining a â€˜Recipe HABTM Tag' relationship, the className key should equal â€˜Tag.'
 ...
 *   The default value for this key is the underscored, singular name of the
 *   current model, suffixed with â€˜_id'.
 ...
 */
	...
	//unset($query[conditions][$this->name.'.'.location_id]);//Gaurav Chauriya
 ...
 */
	public function afterFind($results, $primary = false) {
		if(count($results) > 1){ //show initials in list by pankaj
			
		}else{
			unset($results['Initial']['name']);
			unset($results[0]['Initial']['name']);
		}
		return $results;
	}
