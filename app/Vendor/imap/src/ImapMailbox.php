...
class ImapMailbox {
	...
	public function moveMail($mailId,$folder) {
		return imap_mail_move($this->getImapStream(), $mailId, $folder);
	}
	/**
	...
	protected function initMailPart(IncomingMail $mail, $partStructure, $partNum) {
		...
		if($attachmentId) {
			...
			//	$fileSysName = preg_replace('~[\\\\/]~', '', $mail->id . '_' . $attachmentId . '_' . preg_replace(array_keys($replace), $replace, $fileName));
				$fileSysName = preg_replace('~[\\\\/]~', '', $mail->id  . '_' . preg_replace(array_keys($replace), $replace, $fileName));
