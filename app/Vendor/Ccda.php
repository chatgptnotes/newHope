...
class Ccdas {
	...
	function construct_json() {
		...
		$patient->smoke = $this->smoke ;
		$patient->functionalStatus = $this->functionalStatus ;		 
	...
	private function parse() {
		...
		while(is_object($xmlRoot->component[$i])) {
			...
			// Medications
			if ($test == '2.16.840.1.113883.10.20.22.2.1.1' || $test=='2.16.840.1.113883.10.20.1.8'){ 
				$this->parse_meds($xmlRoot->component[$i]->section);
			}
			// Allergies
			else if ($test == '2.16.840.1.113883.10.20.22.2.6.1' || $test == '2.16.840.1.113883.10.20.1.2') {
			...
			// Immunizations
			else if ($test == '2.16.840.1.113883.10.20.1.6' || $test == '2.16.840.1.113883.10.20.22.2.2' || $test='2.16.840.1.113883.10.20.1.6') {
				$this->parse_immunizations($xmlRoot->component[$i]->section);
			}

			// Labs
			else if ($test == '2.16.840.1.113883.10.20.22.2.3.1' or $test == '2.16.840.1.113883.10.20.22.2.3.1' or
					$test=='2.16.840.1.113883.10.20.22.2.3') {
					
				$this->parse_labs($xmlRoot->component[$i]->section);
			}

			// Problems
			else if ($test == '2.16.840.1.113883.10.20.22.2.5.1' || $test == '2.16.840.1.113883.10.20.22.2.5' || $test== '2.16.840.1.113883.10.20.1.11') {
			...
			// smoking
			else if ($test == '2.16.840.1.113883.10.20.22.2.17') { 
				$this->parse_smoke($xmlRoot->component[$i]->section);
			}
			
			// Functional Status
			else if ($test == '2.16.840.1.113883.10.20.22.2.14' ) {
				$this->parse_functionalStatus($xmlRoot->component[$i]->section);
			}
			...
			// Care Plan
			if ($test == '2.16.840.1.113883.10.20.22.2.10' || $test == '2.16.840.1.113883.3.88.11.83.124') {
	...
	private function parse_meds($xmlMed) {
		foreach($xmlMed->entry as $entry) {
			...
			->end = $this->get_attr(
					...
					'value');
			$this ->rx[$n]
			->product_name_desc = $this->get_attr(
					$entry	->substanceAdministration
					->consumable
					->manufacturedProduct
			...
			$this ->rx[$n]
			->product_name = trim((string) $entry->substanceAdministration ->text);
			
			
			foreach ($entry->substanceAdministration->effectiveTime  as $env){
				$this->rx[$n]->dose_frequency =  $this->get_attr(
						$env
						->period,
						'value');
			}
	...
	private function parse_allergies($xmlAllergy) {
		foreach($xmlAllergy->entry as $entry) {
			...
			foreach($entryRoot->entryRelationship as $detail) {
				...
				if($test == '2.16.840.1.113883.10.20.22.4.28'){
					$varname = 'status';
				}
	...
	private function parse_labs($xmlLab) {
		...
		foreach($xmlLab->entry as $entry) {
			...
			$this ->lab[$n]->panel_name = $this->get_attr(
					$entry	->organizer
					->code,
					'displayName');
			$this ->lab[$n]
			...
			foreach($entry->organizer->component as $component){
				...
				$textRange = (string)$component->observation->referenceRange->observationRange->text;
				if(!$textRange || empty($textRange)){
					$lowRange= $this->get_attr($component->observation->referenceRange->observationRange->value->low,'value');
					$highRange= $this->get_attr($component->observation->referenceRange->observationRange->value->high,'value');
	...
	private function parse_dx($xmlDx) {
		foreach($xmlDx->entry as $entry) {
			...
			->code_system_name = $this->get_attr(
					...
					->translation,
					'codeSystemName');
			foreach($entry->act->entryRelationship
					->observation
					->entryRelationship	as $result){
				$this ->dx[$n]
				->status = $this->get_attr(
						$result->observation->value,'displayName');
			}
	...
	private function parse_careplan($xmlCare) {
		foreach($xmlCare->entry as $entry) {
			...
			elseif (is_object($entry->encounter)) $entryRoot = $entry->encounter;
			else continue;
			...
			->code_system = $this->get_attr(
					...
					'codeSystem');
			$this ->plan[$n]
			->text = (string) $entryRoot ->entryRelationship ->act->text; 
			$this ->plan[$n]
			->status = $this->get_attr(
			...
			$this ->plan[$n]
			->date = $this->get_attr(
					$entryRoot ->effectiveTime->center,
					'value');
	...
	private function parse_smoke($xmlSmoke){
		
		  
		foreach($xmlSmoke->entry as $entry) {
			$n = count($this->smoke);
		  
			$entryRoot = $entry->observation;
			...
			$this ->smoke[$n]
			->low = $this->get_attr(
					$entryRoot ->effectiveTime->low,
					'value');
			
			$this ->smoke[$n]
			->high = $this->get_attr(
					$entryRoot ->effectiveTime->high,
					'value');
		}
	...
	private function parse_functionalStatus($xmlFun){
		
		foreach($xmlFun->entry as $entry) {
			
		//	pr($xmlFun) ;
			//$testFun = $entry->observation->templateId->attributes()->root;
			//if($testFun != '2.16.840.1.113883.10.20.22.4.68') continue ; // skip cognitive status
			
			$n = count($this->functionalStatus);  
			$entryRoot = $entry->observation;
		 
			$this ->functionalStatus[$n]
			->name = $this->get_attr(
					$entryRoot ->code,
					'displayName');
