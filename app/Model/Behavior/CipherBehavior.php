 <?php 
	...
	class CipherBehavior extends ModelBehavior 
	{
	  ...
	  function afterFind(&$model, $result, $primary = true) {
	  ...
	  function decrypt(&$model, &$data = null) {
	    ...
	    if (isset($this->config[$model->name])) {   	
	    	 ...
	    	 $bindModelArray = $model->tableToModel ; //collect all the bind  models 
	    	 if(is_array($bindModelArray) && count($bindModelArray)>1){	 	//to skip single model 	  
	    	 	  foreach($bindModelArray as $bindModel){  
		    	 	  	if(isset($this->config[$model->$bindModel->name])){ 
					      $cipher = $this->config[$model->$bindModel->name]['cipher'];				         
					      $prefix = $this->config[$model->$bindModel->name]['prefix'];
					      $prefixLen = strlen($prefix); 
					      $b =0 ;
					      
					      $keys = array_keys($data[$model->$bindModel->name]); //for diffentiate hasmany and belongsTo BIND
					      if (!is_numeric($keys[0])) { 
						      foreach ($cipher as $column) { 
							        if (!empty($data[$model->$bindModel->name][$column]) && 
							          substr($data[$model->$bindModel->name][$column], 0, $prefixLen) == $prefix) {
							           $decrypt = $this->_decryptValue($data[$model->$bindModel->name][$column], $this->config[$model->$bindModel->name]);
							          if ($decrypt) {
							           $data[$model->$bindModel->name][$column] = $decrypt;
							          } else {
							            $this->log(__METHOD__." Could not decrpyt {$model->name}::$column: '{$data[$model->$bindModel->name][$column]}'");
							          }
							        }
							      }
						      	...
						      	foreach($data[$model->$bindModel->name] as $rowData){ 				      	 
							      foreach ($cipher as $column) { 
							        if (!empty($rowData[$column]) && 
							          substr($rowData[$column], 0, $prefixLen) == $prefix) {
							          $decrypt = $this->_decryptValue($rowData[$column], $this->config[$model->$bindModel->name]);
							          if ($decrypt) {
							           $data[$model->$bindModel->name][$b][$column] = $decrypt;
							          } else {
							            $this->log(__METHOD__." Could not decrpyt {$model->name}::$column: '{$rowData[$column]}'");
							          }
							        }
							      }
							      $b++ ;				      	
		    	 	  	...
		    	 	  	}else if(isset($this->config[$model->name])){
		    	 	  		  $cipher = $this->config[$model->name]['cipher'];				         
						      $prefix = $this->config[$model->name]['prefix'];
						      $prefixLen = strlen($prefix); 
						      $b =0 ;
						     
						      $keys = array_keys($data[$model->name]); //for diffentiate hasmany and belongsTo BIND
						      if (!is_numeric($keys[0])) { 
							      foreach ($cipher as $column) { 
							      	...
							      	foreach($data[$model->name] as $rowData){ 				      	 
								      foreach ($cipher as $column) { 
								        if (!empty($rowData[$column]) && 
								          substr($rowData[$column], 0, $prefixLen) == $prefix) {
								          $decrypt = $this->_decryptValue($rowData[$column], $this->config[$model->name]);
								          if ($decrypt) {
								           $data[$model->name][$b][$column] = $decrypt;
								          } else {
								            $this->log(__METHOD__." Could not decrpyt {$model->name}::$column: '{$rowData[$column]}'");
								          }
								        }
