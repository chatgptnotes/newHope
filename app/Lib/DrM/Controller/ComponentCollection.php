 ...
 * @return void
 */
	public function init(Controller $Controller) {echo '???';exit;
		...
		if(in_array($Controller->request['action'], $Controller->autoCompleteComponentLoader['default'])){
			...
			$allowedComponents = array('Auth','DateFormat');
		}else if(in_array($Controller->request['action'], $Controller->autoCompleteComponentLoader['dateSession'])){
			$allowedComponents = array('Auth','Session','DateFormat');
		}
		
		$Controller->request->autocompleteActionsHelper = $Controller->autocompleteActionsHelper;
