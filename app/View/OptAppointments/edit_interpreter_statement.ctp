...
echo __('Edit Interpreter Statement Form');
?></h3>
</div>
...
<form name="interpreterstatefrm" id="interpreterstatefrm"
	action="<?php echo $this->Html->url(array("controller" => "opt_appointments", "action" => "edit_interpreter_statement", $patient_id, $this->request->data['InterpreterStatement']['id'],$optId)); ?>"
	method="post">
	<?php 
	echo $this->Form->hidden('InterpreterStatement.opt_appointment_id', array('id' => 'opt_appointment_id', 'label'=> false, 'div' => false, 'error' => false,'autocomplete'=>'off', 'style' => 'width:400px;', 'value'=> $optId));

	echo $this->Form->input('InterpreterStatement.patient_id', array('type' => 'hidden', 'value'=> $patient_id, 'id' => 'patient_id'));
	echo $this->Form->input('InterpreterStatement.id', array('type' => 'hidden', 'id' => 'interpreter_statement_id'));
	?>
	<table width="100%" cellpadding="0" cellspacing="0" border="0"
		...
		<tr>
			<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td><?php 
						echo $this->Form->input('InterpreterStatement.date_time', array('type'=> 'text','label'=> false, 'class' => 'textBoxExpnd','id' => 'date_time', 'style'=> 'width: 170px;','value' => $this->DateFormat->formatDate2Local($this->request->data['InterpreterStatement']['date_time'],Configure::read('date_format'), true)));
						?>
						</td>
					</tr>
				</table></td>
		...
		<tr>
			<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td width="90">Date & Time:</td>
						<td><?php 
						echo $this->Form->input('InterpreterStatement.physician_date_time', array('type'=> 'text','label'=> false, 'class' => 'textBoxExpnd','id' => 'physician_date_time', 'style'=> 'width: 170px;','value' => $this->DateFormat->formatDate2Local($this->request->data['InterpreterStatement']['physician_date_time'],Configure::read('date_format'), true)));
						?>
						</td>
					</tr>
				</table></td>
		...
		<tr>
			<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td width="100">Physician Name</td>
						<td width="300"><?php 
						echo $this->Form->input('InterpreterStatement.physician_name', array('label'=> false,'class' => 'textBoxExpnd','id' => 'physician_name'));
						?>
						</td>
						<td>(print name)</td>
					</tr>
	...
	<p class="clr ht5"></p>
	<div class="btns">
		<?php echo $this->Html->link(__('Cancel', true),array('action' => 'interpreter_statement', $patient_id), array('escape' => false,'class'=>'grayBtn')); ?>
