    ...
    <script type="text/javascript">
        $(document).ready(function() {
           ...
           var DATA_FEED_URL = "<?php echo $this->Html->url('/OptAppointments/datafeed/opt_id:'.$opt_id.'/opt_table_id:'.$opt_table_id); ?>";
            var op = {
                view: view,
                theme:3,
                showday: new Date(),
            ...
            function cal_afterrequest(type)
            {
                switch(type)
                {
                    ...
                    case 3:
                    case 4:
                        window.location = "<?php echo $this->Html->url(array("controller" => "opt_appointments", "action" => "otevent"));?>";
            ...
            function Edit(data)
            {
               var eurl="<?php echo $this->Html->url('/opt_appointments/ot_editevent/'); ?>?id={0}&start={2}&end={3}&isallday={4}&title={1}";
                if(data)
                {
                    var url = StrFormat(eurl,data);
                    OpenModelWindow(url,{ width: 1100, height: 600, caption:"Schedule  OT Appointment",onclose:function(){
            ...
            $("#faddbtn").click(function(e) {
                var url ="<?php echo $this->Html->url('/opt_appointments/ot_editevent/'); ?>";
                OpenModelWindow(url,{ width: 600, height: 550, caption: "Schedule New Appointment"});
            });
            //go to today
...
<div class="inner_title">
	<h3>  &nbsp; <?php echo __('Search OT Appointment'); ?></h3>
	<span> <?php echo $this->Html->link(__('Back'),array('action' => 'search'), array('escape' => false,'class'=>'blueBtn')); ?></span>
</div>
<div class="clr">&nbsp;</div>
<?php 
							echo $this->Form->create(null, array('url' => array('controller' => 'opt_appointments', 'action'=>'search_otevent'), 'id'=>'appointmentfrm', 'inputDefaults' => array('label' => false,'div' => false))); 
						?>	
<table border="0" class=""  cellpadding="0" cellspacing="0" width="500px" align="center">
	<tbody>				    			 				    
		<tr class="row_title">				 		
			<td class=""><label><?php echo __('OT Room') ?> :</label></td>		
			<td class="">											 
		    	<?php										  	 
								echo $this->Form->input(null,array('name' => 'opt_id', 'id'=> 'opt_id', 'default' =>  $opt_id, 'empty'=>__('Select OT'),'options'=>$opts,'style' => 'height:30px;',  'onchange'=> $this->Js->request(array('action' => 'search_otevent'),array(
    'before' => $this->Js->get('#busy-indicator')->effect('show'),'complete' => $this->Js->get('#busy-indicator')->effect('hide'), 'async' => true, 'update' => '#changeOptTableList', 'data' => '{opt_id:$("#opt_id").val()}', 'dataExpression' => true, 'div'=>false))));											
			...
			<td class=""><label><?php echo __('OT Table') ?> :</label></td>
			<td class="" id="changeOptTableList">											 
		    	<?php										  	 
								echo $this->Form->input(null,array('name' => 'opt_table_id', 'id'=> 'opt_table_id', 'default' =>  $opt_table_id, 'empty'=>__('Select OT Table'),'options'=>$opttables,'style' => 'height:30px;' ));											
				?>
		  	</td>
...
<div style="text-align: right;" class="clr inner_title"></div>	<br />
    <div>
      <div id="calhead" style="padding-left:1px;padding-right:1px;">
            <div class="cHead"><div class="ftitle"><?php echo __('OT Appointment'); ?></div>
            ...
            <div id="caltoolbar" class="ctoolbar">
              <div id="faddbtn" class="fbutton">
                <div><span title='Click to Create New Event' class="addcal">
                 <?php echo __('Schedule OT Appointment'); ?>
                </span></div>
            </div>
