...
<body>
	<?php 
			$complete_name  = ucfirst($patient['Initial']['name'])." ".ucfirst($patient['Patient']['lookup_name']) ;
	    				...
	    				<td valign="top">
	    					<table>
			    				...
			    				<tr>
			    					...
			    					<td width="100px" valign="top" align="center" rowspan="5">
			    						<?php echo $this->Html->image("/uploads/qrcodes/".$patient['Patient']['patient_id'].".png", array('alt' => $complete_name,'title'=>$complete_name,'width'=>'100')); ?>
			    					</td>
			 					</tr>
			 					<?php  } ?>
			 				</table>
...
<table class="table_format"  border="0" cellpadding="0" cellspacing="0" width="100%" style="text-align:center;">
  ...
  <?php #echo '<pre>';print_r($patientsData);exit;
   ...
   if($isAssigned == 0){
  		...
  		echo '<td>';
  		echo $this->Form->create('Bed',array('action'=>'assignBed'));
   		echo $this->Form->hidden('patient_id',array('value'=>$id));
   		echo $this->Form->hidden('room_id',array('value'=>$data['Room']['id']));
   		echo $this->Form->hidden('bed_id',array('value'=>$i));
  		echo $this->Form->submit(__('Assign', true), array('class' => 'blueBtn','div'=>false,'label'=>false));
  		echo $this->Form->end();
  		echo '</td>';
  		echo '<td>&nbsp;</td>';
  		echo '<td>&nbsp;</td>';
